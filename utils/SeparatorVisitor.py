...
class SeparatorVisitor:
    ...
    def on_visit(self, node):
        ...
        """
        # visitor_fn = getattr(self, f"visit_{node.type}", self.visit) # assembly_statement
        # print(node.type)
        # print(node.text)
        if node.type in ["return_statement", "variable_declaration_statement", "expression_statement", "state_variable_declaration"]:
            startnode = node
            while( len(startnode.children) !=0 ):
                startnode=startnode.children[-1]
            self.append_comma_newline.append((startnode.start_point, startnode.end_point))
        elif node.type in ["function_definition", "assembly_statement"]:
            startnode = node
            while( len(startnode.children) !=0 ):
                startnode=startnode.children[-1]
            self.append_newline.append((startnode.start_point, startnode.end_point))
        elif node.type in [ "import_directive"  , "pragma_directive" , "using_directive" ]:
            startnode = node
            while( len(startnode.children) !=0 ):
                startnode=startnode.children[-1]
            self.append_comma_newline.append((startnode.start_point, startnode.end_point))
            ...
            startnode = node
            self.append_newline.append((startnode.start_point, startnode.end_point))
        elif node.type in ["struct_member"]:
            startnode = node
            while( len(startnode.children) !=0 ):
                startnode=startnode.children[-1]
            self.append_comma_newline.append((startnode.start_point, startnode.end_point))
